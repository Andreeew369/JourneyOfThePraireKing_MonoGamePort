...
public class Player : GameObject {
    ...
    private const int DefualtHealth = 3;
    ...
    /// <summary>
    /// Index in sprite array
    /// </summary>
    private int _sprite;

    public IPowerUp InventoryPowerUp { get; set; }
    //public IPowerUp ActivePowerUp { get; set; } 
    private ImmutableHashSet<IPowerUp> ActivePowerUps => _activePowerUps.ToImmutableHashSet();
    ...
    /// <summary>
    /// Updates the list of Game Objects in players surroundings
    /// </summary>
    ...
    /// non-zero value. This used for detecting collision in one direction at a time and to know which direction to check. If one is 0 then it knows
    ...
    /// <param name="differenceOut">Distance the player can move in the next move</param>
    /// <returns>True if the player is colliding with a Game Object that player can't move through</returns>
    public bool CollisionDetection(float nextX, float nextY, float velocity, out float differenceOut) {
        RectangleF hitbox = HitBox;
        List<GameObject> copy = new(_surroundings);
        foreach (GameObject o in copy) {
            ...
            differenceOut = Functions.GetDistanceBetweenObjects(nextX, nextY, this, o, velocity);
        ...
        differenceOut = velocity;
        return false;
    }
    
    public bool DoesPlayerHavePowerUp(IPowerUp powerUp, out IPowerUp? powerUpOut) {
        powerUpOut = ActivePowerUps.FirstOrDefault(pu => pu.GetType() == powerUp.GetType());
