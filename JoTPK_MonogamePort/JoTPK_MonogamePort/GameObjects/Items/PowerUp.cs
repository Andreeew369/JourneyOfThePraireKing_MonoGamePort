...
/// <summary>
/// Interface for power ups. Recommended to inherit from GameObject too. 
/// </summary>
public interface IPowerUp : IItem {
    ...
    /// <param name="player">Instance of player in current game</param>
    public static void GlobalDeactivate(IPowerUp powerUp, Player player) => player.RemoveActivePowerUp(powerUp);
    ...
    public static void GlobalPickup(IPowerUp powerUp, Player player, Level level) {
        if (powerUp is not GameObject goPowerUp)
            throw new ArgumentException("Invalid power up. Power up has to be instance of GameObject and IItem");
